...
export default function ListingDetailPage() {
  ...
  const loadListing = async () => {
    try {
      ...
      if (sellerError || !sellerRows || sellerRows.length === 0) {
        setSeller({ full_name: 'Unknown Seller', department: '', semester: '', is_verified: false });
      } else {
        setSeller(sellerRows[0]);
      }

      const { data: reviewsData } = await supabase
        .from('reviews').select('*').eq('seller_id', listingData.user_id)
  ...
  const handleRevealWhatsapp = async () => {
    ...
    try {
      ...
      await supabase.from('contacts').upsert(
        [{ user_id: session.user.id, contact_user_id: listing.user_id,
           contact_name: seller?.full_name || 'Seller', contact_whatsapp: whatsapp, contact_type: 'seller' }],
  ...
  return (
    <div className="min-h-screen bg-white pb-36">
          ...
          <div className="space-y-2">
            {isSold ? (
              <div className="w-full py-3 px-4 rounded-lg bg-red-50 border border-red-200 text-center">
                <p className="text-sm text-red-600 font-semibold">This item has been sold</p>
              </div>
            ) : isOwner ? (
              ...
              <button onClick={handleRevealWhatsapp} disabled={revealLoading}
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {currentUser && !isOwner && (
              <button onClick={() => setShowRatingModal(true)}
                ...
                style={{ background: '#EFF6FF', color: '#1877F2' }}>+ Review</button>
            )}
          </div>
          {reviews.length === 0 ? (
            <div className="text-center py-6">
              ...
              {currentUser && !isOwner && (
                <button onClick={() => setShowRatingModal(true)}
                  className="text-sm font-semibold" style={{ color: '#1877F2' }}>Be the first to review</button>
              )}
            </div>
          ) : (
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end md:items-center justify-center px-4 pb-6">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleMarkAsSold} disabled={soldLoading}
                ...
                style={{ background: 'linear-gradient(135deg, #16A34A, #22C55E)' }}>
                {soldLoading ? <><Loader className="w-4 h-4 animate-spin" /> Updating...</> : 'Mark Sold'}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm">
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowDeleteConfirm(false)} disabled={deleteLoading}
                className="flex-1 py-3 rounded-xl font-semibold text-sm border-2 border-gray-200"
                style={{ color: '#6B7280' }}>Cancel</button>
              <button onClick={handleDeleteListing} disabled={deleteLoading}
                className="flex-1 py-3 rounded-xl font-semibold text-sm text-white flex items-center justify-center gap-2 disabled:opacity-50"
      ...
      {showRatingModal && (
        <RatingModal sellerId={listing.user_id} listingId={listing.id}
          onClose={() => setShowRatingModal(false)} onSuccess={() => loadListing()} />
      )}
      {showReportModal && (
