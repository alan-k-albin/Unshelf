...
import { 
  ...
  Search as SearchIcon, ArrowRight, Shield, Zap, Users, Home, MessageCircle, Plus, User
} from 'lucide-react';
...
export default function Home() {
  ...
  return (
    <div className="pb-28 md:pb-8 bg-gradient-to-b from-slate-50 via-blue-50 to-white min-h-screen">
      {/* ===== HERO SECTION ===== */}
      <section className="relative px-4 pt-6 pb-8 md:pt-12 md:pb-12 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto">
          {/* Logo */}
          <div className="inline-flex items-center justify-center w-11 h-11 rounded-xl mb-3 bg-gradient-to-br from-indigo-600 to-indigo-700 shadow-lg mx-auto block">
            <BookOpen className="w-5 h-5 text-white" />
          </div>

          {/* Hero Text */}
          <h1 className="text-3xl md:text-4xl font-bold text-center mb-2.5 tracking-tight" style={{ color: '#1B2A4A' }}>
            Your College Marketplace
          </h1>

          <p className="text-center text-gray-600 mb-6 text-sm max-w-2xl mx-auto leading-relaxed">
      ...
      {personalizedListings.length > 0 && (
        <section className="px-4 py-6 md:py-10 max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href={`/search?department=${userDept}`} className="text-sm font-semibold transition" style={{ color: '#4F46E5' }}>
              View All →
            </Link>
          </div>
      ...
      <section className="px-4 py-6 md:py-10 max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/search" className="text-sm font-semibold transition" style={{ color: '#4F46E5' }}>
            Explore →
          </Link>
        </div>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-40 backdrop-blur-xl bg-white/90">
        <div className="flex justify-around items-end relative">
          {/* Home */}
          <Link href="/" className="flex-1 flex flex-col items-center justify-center py-3 transition text-indigo-600 font-semibold">
            <Home className="w-6 h-6" />
            <span className="text-xs mt-1">Home</span>
          </Link>

          {/* Chats */}
          <Link href="/chats" className="flex-1 flex flex-col items-center justify-center py-3 text-gray-600 hover:text-gray-900 transition">
          ...
          {/* Account */}
          <Link href={isLoggedIn ? '/profile' : '/login'} className="flex-1 flex flex-col items-center justify-center py-3 text-gray-600 hover:text-gray-900 transition">
