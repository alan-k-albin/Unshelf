    ...
    <div className="group cursor-pointer h-full rounded-2xl overflow-hidden bg-white border border-gray-100 hover:border-gray-200 hover:shadow-lg transition-all duration-300">
      <div className="relative w-full h-40 bg-gradient-to-br from-gray-100 to-gray-200 overflow-hidden">
        ...
        <div className={`absolute top-3 right-3 text-xs font-bold px-2.5 py-1 rounded-full backdrop-blur-sm ${
          listing.status === 'Active' ? 'bg-green-100/80 text-green-700' : 'bg-red-100/80 text-red-700'
        }`}>
          {listing.status === 'Active' ? '●' : '✓'} {listing.status}
        </div>
        {listing.featured && (
          <div className="absolute top-3 left-3 bg-yellow-400/90 text-yellow-900 text-xs font-bold px-2.5 py-1 rounded-full">⭐</div>
        )}
      </div>
      <div className="p-3.5">
        <h3 className="font-semibold text-sm line-clamp-2 mb-2" style={{ color: '#1B2A4A' }}>{listing.title}</h3>
        <div className="mb-2.5">
          <span className="inline-block bg-indigo-100 text-indigo-700 text-xs px-2 py-1 rounded-full font-medium">{listing.category}</span>
        </div>
        <div className="flex gap-1.5 mb-2.5 text-xs">
          {listing.department && <span className="bg-gray-50 text-gray-600 px-2 py-1 rounded-full">{listing.department}</span>}
          {listing.semester && <span className="bg-gray-50 text-gray-600 px-2 py-1 rounded-full">{listing.semester}</span>}
        </div>
...
const CategoryCard = ({ name, categoryInfo }) => {
  ...
  return (
    <Link href={`/search?category=${name}`}>
      <div className="group relative overflow-hidden rounded-2xl p-5 transition-all duration-300 hover:shadow-xl cursor-pointer h-32 flex flex-col items-center justify-center active:scale-95"
        style={{ background: categoryInfo?.lightBg }}>
        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
          style={{ background: categoryInfo?.darkBg }} />
        <div className="relative z-10 text-center">
          <div className="w-12 h-12 rounded-xl mb-2.5 flex items-center justify-center mx-auto text-white transition-all duration-300 group-hover:scale-110"
            style={{ background: categoryInfo?.darkBg }}>
            <IconComponent className="w-6 h-6" />
          </div>
          <p className="font-semibold text-sm transition-colors duration-300 group-hover:text-white line-clamp-2"
            style={{ color: categoryInfo?.darkBg }}>{name}</p>
        </div>
      </div>
    </Link>
  );
    ...
    <div className="bg-gradient-to-b from-slate-50 via-blue-50 to-white min-h-screen">
      ...
      <section className="relative px-4 pt-6 pb-8 md:pt-12 md:pb-12 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center">
          ...
          {!isLoggedIn && <p className="text-xs text-gray-500 mt-2">Sign up free • No payment needed</p>}
        </div>
      </section>
      ...
      {personalizedListings.length > 0 && (
        <section className="px-4 py-6 md:py-10 max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg md:text-xl font-bold" style={{ color: '#1B2A4A' }}>📚 For {userDept}</h2>
      ...
      <section className="px-4 py-6 md:py-10 max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg md:text-xl font-bold" style={{ color: '#1B2A4A' }}>Latest Listings</h2>
          <Link href="/search" className="text-sm font-semibold" style={{ color: '#4F46E5' }}>Explore →</Link>
        </div>
      ...
      <section className="px-4 py-8 md:py-12 max-w-7xl mx-auto">
        <h2 className="text-2xl font-bold text-center mb-6" style={{ color: '#1B2A4A' }}>Why Unshelf?</h2>
